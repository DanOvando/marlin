  ...
  for (s in 2:steps) {
    ...
    for (l in seq_along(fleet_names)) {
      ...
      last_profits <-
        last_revenue - fleets[[l]]$cost_per_unit_effort * sum((fleets[[l]]$e_p_s[, s - 1] + 1) ^
                                                                fleets[[l]]$effort_cost_exponent) # calculate profits in the last time step
    ...
    for (f in seq_along(fauni)) {
      ...
      if ((length(habitat) > 0) &
        ...
        current_habitat <-
          exp(outer(log(current_habitat + 1e-6), log(current_habitat + 1e-6), "-")) # calculate difference in habitat between each patch
      ...
      for (fl in 1:length(fleets)) {
        ...
        prof_p_fl[, fl] <-
          r_p_fl[, fl] - fleets[[fl]]$cost_per_unit_effort * (as.matrix(tmp_e_p_fl[, fl] +
                                                                          1) ^ fleets[[fl]]$effort_cost_exponent) / length(fauna)
