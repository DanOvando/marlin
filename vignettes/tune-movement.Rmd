...
n_next <- n %*% expm(time_steps * diffusion_matrix)
...
grid |> 
  ...
  geom_hline(yintercept = resolution / 2 + c(D / 2, - D/2), color = "red") +
  geom_vline(xintercept = resolution / 2 + c(D / 2, - D/2), color = "red")
...
Let's put some prime habitat over in the far left of the grid

```{r}

habitat <- rep(100, patches)

habitat[grid$x == 1 & grid$y == resolution / 2] <- 200
...
taxis_matrix <-  adjacent *  (D * (delta_t / delta_d^2)) *  exp(habitat_difference * delta_t / delta_d)  # adjacency times taxis matrix 
...
n_next <- n %*% expm(100 * taxis_matrix)
