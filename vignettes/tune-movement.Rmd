...
Under this model, movement from each patch to each other patch in the system in a given timestep
...
matrix exponent of a function of the diffusion $\boldsymbol{D}$ and
taxis $\boldsymbol{\tau}$ matrices scaled by the width of the time step
$\Delta_{t}$ and the length of the edge of each patch $\Delta_d$ that
the model is running under. So for example, if we are running a model on
a monthly timestep, and we define the units of time in years, then
$\Delta_{t} = 1/12$. Similarly, if we are running a model where one
patch equals 100km^2^, then $\Delta_d = 10km$. This parameterization
...
$$
\boldsymbol{M}_{t,s,a} = e^{\Delta_{t}(\boldsymbol{D}_{t,s,a} + \boldsymbol{\tau}_{t,s,a})} 
$$ {#eq-movement}

The individual components (*m*) of the movement matrix are filled based
...
time step and patch size used in a simulation. This "scale free"
...
off-diagonal elements of the movement matrix $\boldsymbol{M}_{t,s,a}$
...
Let's set
  ...
  * a diffusion parameter *D* of 20 km^2^/year
...
grid |>
  ...
  geom_tile(
    aes(
      x = resolution / 2 * delta_d ,
      y = resolution / 2 * delta_d,
      width = sqrt(D),
      height = sqrt(D)
    ),
    alpha = 0.1,
...
grid |> 
  ggplot() + 
  geom_tile(aes(x * delta_d,y * delta_d,fill = n)) + 
  scale_fill_viridis_c(name = "Number of Fish") + 
  scale_x_continuous(name = "Distance from Bottom Left Corned in KM", expand = c(0,0)) + 
  scale_y_continuous(name = "Distance from Bottom Left Corned in KM", expand = c(0,0))
