...
The previous scenarios all show smooth, well-behaved dynamics. None of them visibly oscillate. But structural perturbations — most commonly an MPA closure that abruptly shifts the open-patch set — can excite the one-step-lag feedback discussed in the algorithm section, producing visible period-2 sawtooth in patch-level effort.

Here we set up a 2-fleet system (cost-aware allocation, ppue / profit) and close an offshore corner mid-run, then sweep `objective_memory_halflife` across a small range. The expectation: at `halflife = 0` (legacy behavior) the post-MPA dynamics show high-frequency wiggle in individual patches; modest memory dampens it cleanly; very long memory introduces a different problem — low-frequency overshoot driven by the phase lag the smoothing imposes.
...
Reading the patch traces: `halflife = 0` shows the high-frequency wiggle that motivates the parameter — clearest under cost-aware allocation, most visible after the MPA closure structurally shifts where each fleet can fish. `halflife = 1` (one year of memory) sits near the median of the sawtooth and removes the high-frequency component. `halflife = 2` smooths further but typically begins to introduce slower swings driven by phase lag: the fleet's perceived objective is lagging the real one by ~3 years, so it reallocates toward patches that *used to be* high-value.

Sensible values for `objective_memory_halflife` depend on the time scale of stock and fleet dynamics. For most marine fisheries with annual or sub-annual seasons, **0.5–1.5 years** is a useful starting range. If catch trajectories at a chosen halflife show slow swings that aren't present at `halflife = 0`, reduce it; you've crossed into the phase-lag regime.
