...
We will now project this population with an MPA
...
ssb_mako <- rowSums(mpa_sim[[steps]]$mako$ssb_p_a)
...
ssb_yft <- rowSums(mpa_sim[[steps]]$yft$ssb_p_a)
...
mako_outside_trajectory <- map_dbl(mpa_sim,~ sum(.x$mako$ssb_p_a[mpa_locations$mpa == FALSE,]))

plot(mako_outside_trajectory)

mako_inside_trajectory <- map_dbl(mpa_sim,~ sum(.x$mako$ssb_p_a[mpa_locations$mpa == TRUE,]))

plot(mako_inside_trajectory)

yft_outside_trajectory <- map_dbl(mpa_sim,~ sum(.x$yft$ssb_p_a[mpa_locations$mpa == FALSE,]))

plot(yft_outside_trajectory)

yft_inside_trajectory <- map_dbl(mpa_sim,~ sum(.x$yft$ssb_p_a[mpa_locations$mpa == TRUE,]))

plot(yft_inside_trajectory)

```

Now, create scenario but move sharks offshore
